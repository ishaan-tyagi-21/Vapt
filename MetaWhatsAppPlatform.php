    ...
    public function sendNotification(
        ...
        if ($filledTemplate instanceof Result) {
            return new PlatformNotificationSendResult(
                ...
                $filledTemplate->msg ?? $filledTemplate->code
            );
        }

        // Log document/media URLs for debugging delivery issues
        foreach ($filledTemplate as $component) {
            if (($component['type'] ?? '') === 'header') {
                foreach ($component['parameters'] ?? [] as $param) {
                    if (isset($param['document']['link']) || isset($param['image']['link'])) {
                        $mediaUrl = $param['document']['link'] ?? $param['image']['link'] ?? '';
                        lkn_hn_debug_trace('MetaWhatsApp: sending document/media URL', [
