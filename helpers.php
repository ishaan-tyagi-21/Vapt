...
/**
 * Debug trace logger - writes directly to tblmodulelog bypassing WHMCS Module Debug Mode.
 * View logs in the module UI: Help > Logs (filter by "[DEBUG]")
 */
function lkn_hn_debug_trace(string $step, array $data = []): void
{
    try {
        $dataStr = !empty($data) ? json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR) : '';

        Capsule::table('tblmodulelog')->insert([
            'module' => 'lknhooknotification',
            'action' => '[DEBUG] ' . $step,
            'request' => $dataStr,
            'response' => '',
            'arrdata' => '',
...
function lkn_hn_config(Settings $setting)
{
    ...
    $parsed_value = match ($setting) {
        ...
        Settings::INVOICE_RENEWAL_CONFIG => $value ? json_decode($value, true) : null,
        default => $value
    };
