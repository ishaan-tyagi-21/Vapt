...
final class LogsController extends BaseController
{
    ...
    {
        if (isset($request['clear-debug-logs'])) {
            $this->logsRepository->clearDebugLogs();
            header('Location: ?module=lknhooknotification&page=logs');
            exit;
        }
        ...
        foreach ($logsForView['logs'] as $log) {
            try {
                ...
                lkn_hn_log('logs_controller_render_error', ['log_id' => $log->id ?? null], $th->__toString());
            }
        }
